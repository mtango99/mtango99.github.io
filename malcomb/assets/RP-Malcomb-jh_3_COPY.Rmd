...
#joined table in QGIS, so this lhz layer has the relevant attributes for calculating livelihood sensitivity
lhz = read_sf(here("data", "derived", "private", "MW_LHZ_2009_derived.shp")) %>% st_make_valid()

#### adding traditional authority and livelihood zone ids to dhs clusters ####
dhsclusters_2010 = shape %>% 
  st_transform(3395) %>% 
...
#geo is one layer parameter, in this case, we will call the vulnerability function later on the rasterized
#ta_capacity layer
...
fig4compare = inner_join(rp_fig4,or_fig4,by="ID_2") %>%  
  ...
  filter(rp_res>0 & rp_res<5 & resilience2 > 0 & resilience2 < 5)
  # keep only records with valid resilience scores

table(fig4compare$resilience2,fig4compare$rp_res)
# crosstabulation with frequencies

cor.test(fig4compare$resilience2,fig4compare$rp_res,method="spearman")
# Spearman's Rho correlation test

fig4compare = mutate(fig4compare, difference = rp_res - resilience2) 
# Calculate difference between the maps so that you can create a difference map

results2 = here("data","derived","public","fig4compare2.gpkg")
write_sf(fig4compare, results2, "fig4compare2")
...
```{r compare raster maps}
#COMPARE RASTER MAPS

orfig5vect = 
  read_sf(here("data", "derived", "public", "georeferencing_grid.gpkg"), 
          layer="georeferencing_grid")
# load original figure 5 data

orfig5rast = st_rasterize(orfig5vect["mean"], template=ta_final)
...
orfig5rast = orfig5rast %>% 
  mutate(or = 1-
           (mean - min(orfig5rast[[1]], na.rm= TRUE)) /
...
write_stars(fig5comp["diff"], here("data","derived","public","fig5comp.tif"))
#join to raster?? tcapacity
...
map_fig4compare = ggplot() +
  ...
  geom_sf(
    ...
    scale_fill_manual(limits = c("-2","-1","0","1","2","Missing Data","Major Lakes of Malawi","National Parks and Reserves"), 
                    values = c("-2"="darkred","-1"="red","0"="grey","1"="green","2"="blue","Missing Data"="#000000","Major Lakes of Malawi"="lightblue","National Parks and Reserves"="#D9EABB"))+ #thanks Vincent
  scale_x_continuous(breaks = c(33,34,35,36)) +  
  labs(title = "Difference in Adaptive Capacity Scores Based on 2010 DHS Surveys in 197 Traditional Authorities") +
  theme_minimal() +
  theme(legend.title = element_blank())
...
```{r diff vulnerability map fig5comp}
...
map_fig5comp = ggplot() +
  ...
  scale_fill_gradient(
    low = "#FFFF75",
    high = "#CF4611",
    breaks = c(-0.8176,0.6141),
    labels = c("RP Vunerability Lower", "RP Vulnerability Higher"),
    na.value = "transparent",
    guide = "colourbar",
    limits = c(-0.8176,0.6141)
  ) +
  scale_x_continuous(breaks = c(33,34,35,36)) +
  labs(title = "Difference in Malawi Vulnerability to Climate Change") +
  theme_minimal() +
  theme(
    legend.title = element_blank(),
    axis.title.x = element_blank(),
...
```{r saving difference maps}

save(map_fig4compare, map_fig5comp, file = here("results","maps","diffmaps.Rdata"))

ggsave(
  here("results","maps","map_fig4compare.png"),
  plot = map_fig4compare,
  width = 8.5,
  height = 11,
  units = "in"
)
ggsave(
  here("results","maps","map_fig5comp.png"),
  plot = map_fig5comp,
  width = 8.5,
  height = 11,
  units = "in"
